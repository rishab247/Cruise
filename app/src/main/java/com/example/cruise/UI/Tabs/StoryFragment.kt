...
class StoryFragment : Fragment() ,GLSurfaceView.Renderer {

    private val TAG: String = StoryFragment::class.java.simpleName
    ...
    override fun onCreateView(
        ...
        trackingStateHelper = activity?.let { TrackingStateHelper(it) }!!
        displayRotationHelper = context?.let { DisplayRotationHelper(it) }!!
    ...
    override fun onResume() {
        ...
        try {
            ...
            activity?.let { messageSnackbarHelper.showError(it, getString(R.string.camera_not_available)) }
            session = null
            return
        }
    ...
    private fun setupSession(): Boolean {
        ...
        if (message != null) {
            activity?.let { messageSnackbarHelper.showError(it, message) }
    ...
    fun onWindowFocusChanged(hasFocus: Boolean) {
        ...
        activity?.let { FullScreenHelper.setFullScreenOnWindowFocusChanged(it, hasFocus) }
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: javax.microedition.khronos.egl.EGLConfig?) {
        ...
        try {
            ...
            context?.let { planeRenderer.createOnGlThread(it, getString(R.string.model_grid_png)) }
        ...
        if (camera.trackingState == TrackingState.PAUSED) {
            activity?.let {
                messageSnackbarHelper.showMessage(
                        it, TrackingStateHelper.getTrackingFailureReasonString(camera)
                )
            }
            return false
        }
    ...
    private fun checkPlaneDetected() {
        if (hasTrackingPlane()) {
            activity?.let { messageSnackbarHelper.hide(it) }
        } else {
            activity?.let {
                messageSnackbarHelper.showMessage(
                        it,
